...
##########################################
# D3vOS Configuration Comparison Tool
# Author: Don Williams
# Date: $(date +"%B %d, %Y")
##########################################
...
# Define directories
ZANEYOS_DIR="$HOME/d3vos"
TEMP_DIR="/tmp/d3vos-comparison-$$"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
REPORT_FILE="$HOME/d3vos-config-comparison-$TIMESTAMP.txt"

# Default comparison targets
DEFAULT_REPO="https://gitlab.com/d3vboi/d3vos.git"
DEFAULT_BRANCH="main"
...
show_usage() {
  echo -e "${CYAN}D3vOS Configuration Comparison Tool${NC}"
  echo ""
  echo "Usage: $0 [OPTIONS]"
  echo ""
  echo "Options:"
  echo "  -r, --repo URL          Remote repository URL to compare against"
  echo "                          (default: https://gitlab.com/d3vboi/d3vos.git)"
  ...
  echo "  $0 -r https://github.com/user/d3vos.git -b dev"
...
perform_comparison() {
  print_header "D3vOS Configuration Comparison Tool"
  ...
  # Check if we're in a d3vos directory
  if [ ! -d "$ZANEYOS_DIR" ]; then
    print_error "D3vOS directory not found at $ZANEYOS_DIR"
    exit 1
  fi
  ...
  if [ ! -d ".git" ]; then
    print_warning "Current D3vOS directory is not a git repository"
    print_info "This tool works best when your D3vOS is version controlled"
  fi
  ...
  {
    echo "============================================"
    echo "  D3vOS Configuration Comparison Report"
    echo "  Generated: $(date)"
    ...
    if [ -d ".git" ]; then
      ...
      echo "   git init"
      echo "   git add ."
      echo "   git commit -m 'Initial D3vOS configuration'"
    fi
    echo ""
